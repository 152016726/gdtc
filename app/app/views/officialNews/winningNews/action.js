...
import {PAGE_SIZE, DEVICER_TYPE, WINNINGNEWS_NODEID} from '~/constants/articleConfig';
...
let actions = {
    ...
    reqData(pageIndexNum = 0) {
        return ((dispatch, getState) => {
            let {informationList} = getState()[storeKey.WINNING_STORE];
            let isFooter = false;
            let defNodeReq = {
                nodeIds: [WINNINGNEWS_NODEID],
    ...
    pullUpLoad() {
        return ((dispatch, getState) => {
            let {pageIndex} = getState()[storeKey.WINNING_STORE];
            pageIndex++;
    ...
    updateData: createAction(ActionTypes.UPDATE_WINNING, (dataObj) => dataObj)
};

...
import {EXPERT_NODEID, PAGE_SIZE, DEVICER_TYPE} from '../../../constants/articleConfig';
...
let actions = {
    ...
    reqData(pageIndexNum = 0) {
        return ((dispatch, getState) => {
            let {informationList} = getState()[storeKey.EXPERT_STORE];
            let isFooter = false;
            let defNodeReq = {
    ...
    pullUpLoad() {
        return ((dispatch, getState) => {
            let {pageIndex} = getState()[storeKey.EXPERT_STORE];
            pageIndex++;
    ...
    updateData: createAction(ActionTypes.UPDATE_EXPERT, (dataObj) => dataObj)
};
